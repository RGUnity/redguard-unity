...
namespace xyz
{
    public class test
	{
		...
		{
            List<string> rgms = new List<string> {
                "START",
                "DRINT",
                "PALACE",
                "ISLAND",
            ...
            Console.WriteLine($"Dictionary<string, List<uint>> WorldObjects = new Dictionary<string, List<uint>> {{");
			RGRGMFile filergm = new RGRGMFile();
            for(int i=0;i<rgms.Count;i++)
            {
                Console.WriteLine($"\t{{\"{rgms[i]}\" , new List<uint>{{");

                filergm.LoadFile($"../../game_3dfx/maps/{rgms[i]}.RGM");

                for(int j=0;j<filergm.MPOB.num_items;j++)
                {
                    RGRGMFile.RGMMPOBItem it = filergm.MPOB.items[j];
                    Console.WriteLine($"\t\t// 0x{it.id:X}, // {it.scriptName}");
                }
                Console.WriteLine($"\t\t}}");
                Console.WriteLine($"\t}},");
            }
...
Dictionary<string, List<uint>> WorldObjects = new Dictionary<string, List<uint>> {
	{"START" , new List<uint>{
		// 0x8384DD60, // SHARK
		// 0x8384DDA8, // WATERSND
		// 0x8384DEE8, // SHARK
